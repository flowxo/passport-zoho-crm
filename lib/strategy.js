 ...
 */
 function Strategy(options, verify) {
  ...
  this._oauth2.getOAuthAccessToken = function(code, params, callback) {
    if (code.substring(0,9) === 'location:'){
      let loc = code.substring(9,11);
      self._oauth2._accessTokenUrl = self._oauth2._accessTokenUrl.replace('.com',`.${loc}`);
...
Strategy.prototype.authenticate = function(req, options) {
 if (req.query.location && req.query.location !== 'us'){
   this._userProfileURL = this._userProfileURL.replace('.com',`.${req.query.location}`);
   this._oauth2.__location = req.query.location;
   this._oauth2._clientSecret = this._oauth2.__euClientSecret;
   this._oauth2._accessTokenUrl = this._oauth2._accessTokenUrl.replace('.com',`.${req.query.location}`);
 }
  let self = this;
